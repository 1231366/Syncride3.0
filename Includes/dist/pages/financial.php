...
try {
    ...
    $catLabels = [];
    $catValues = [];
    $receitaEstimada = 0; $totalDespesas = 0; $lucroLiquido = 0;
}
?>
...
<head>
    ...
    <style>
        ...
        [data-bs-theme="dark"] input[type="month"]::-webkit-calendar-picker-indicator { filter: invert(1); }
        ...
        @media (max-width: 991.98px) {
            ...
            #tabelaDespesas tbody td { display: block; border: none !important; padding: 2px 0; width: 100% !important; margin-bottom: 4px; }
            
            /* Data e Categoria */
            #tabelaDespesas tbody td:nth-child(1) { font-size: 0.75rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 4px; }
            ...
            /* Valor (Topo Direito) */
            #tabelaDespesas tbody td:nth-child(4) { position: absolute; top: 12px; right: 12px; width: auto !important; font-size: 1.1rem; font-weight: 800; color: #ef4444; text-align: right; }
            
            /* Ações */
            #tabelaDespesas tbody td:last-child {
...
<body class="layout-fixed sidebar-expand-lg">
    <div class="app-wrapper">
      ...
      <div class="bottom-navbar d-lg-none shadow-lg">
        ...
        <a href="financial.php" class="nav-item-bottom active"><i class="bi bi-cash-coin"></i><span>Finanças</span></a>
        ...
        <div class="app-content">
          <div class="container-fluid">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-4 gap-3">
                <div><h3 class="fw-bold mb-0 text-main">Financeiro</h3><p class="text-muted mb-0 small">Controlo de receitas e despesas.</p></div>
                <div class="d-flex gap-2 w-100 w-md-auto align-items-center">
                    <form method="GET" class="flex-grow-1 flex-md-grow-0">
                        <input type="month" name="month" class="form-control-custom shadow-sm" value="<?php echo $mesFiltro; ?>" onchange="this.form.submit()" style="min-width: 150px;">
                    </form>
                    <button class="btn btn-modern shadow-sm text-nowrap" data-bs-toggle="modal" data-bs-target="#modalExpense"><i class="bi bi-plus-lg me-1"></i> <span class="d-none d-sm-inline">Despesa</span></button>
                </div>
            </div>

            <div class="row g-4 mb-4">
                <div class="col-md-4 col-12"><div class="stat-card stat-green"><div><div class="stat-icon-wrapper"><i class="bi bi-graph-up-arrow"></i></div><div class="stat-value"><?= number_format($receitaEstimada, 0, ',', '.'); ?>€</div><div class="stat-label">Receita Estimada</div></div></div></div>
                <div class="col-md-4 col-6"><div class="stat-card stat-red"><div><div class="stat-icon-wrapper"><i class="bi bi-cart-dash"></i></div><div class="stat-value"><?= number_format($totalDespesas, 0, ',', '.'); ?>€</div><div class="stat-label">Despesas</div></div></div></div>
                <div class="col-md-4 col-6"><div class="stat-card stat-blue"><div><div class="stat-icon-wrapper"><i class="bi bi-wallet2"></i></div><div class="stat-value"><?= number_format($lucroLiquido, 0, ',', '.'); ?>€</div><div class="stat-label">Lucro Líquido</div></div></div></div>
            </div>

            <div class="row g-4">
                <div class="col-lg-8 connectedSortable">
                    <div class="card h-100 border-0">
                        <div class="card-header d-flex justify-content-between align-items-center"><h3 class="card-title fw-bold">Despesas</h3><span class="badge bg-light text-muted border"><?= count($listaDespesas); ?> registos</span></div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table id="tabelaDespesas" class="table table-hover w-100 m-0 align-middle">
                                    <thead>
                                        <tr><th class="ps-4">Data</th><th>Categoria</th><th>Descrição</th><th class="text-end">Valor</th><th class="text-center pe-4">Ações</th></tr>
                                    </thead>
                                    <tbody>
                                                ...
                                                <td class="text-center pe-4">
                                                    <div class="d-flex gap-2 justify-content-end">
                                                        <?php if($d['file_path']): $ext=strtolower(pathinfo($d['file_path'], PATHINFO_EXTENSION)); ?>
                                                            <button class="btn-action" onclick="openPreview('<?php echo htmlspecialchars($d['file_path']); ?>','<?php echo $ext; ?>')" title="Ver"><i class="bi bi-eye"></i></button>
                                                        <?php endif; ?>
                                                        <button class="btn-action edit-btn" data-id="<?php echo $d['id']; ?>" data-cat="<?php echo $d['category']; ?>" data-desc="<?php echo $d['description']; ?>" data-amount="<?php echo $d['amount']; ?>" data-date="<?php echo $d['date']; ?>" title="Editar"><i class="bi bi-pencil"></i></button>
                                                        <a href="save_expense.php?action=delete&id=<?php echo $d['id']; ?>" class="btn-action delete" onclick="return confirm('Apagar?');" title="Apagar"><i class="bi bi-trash"></i></a>
                                                    </div>
                                                </td>
                                            </tr>
                                            <?php endforeach; ?>
                ...
                <div class="col-lg-4 connectedSortable">
                    <div class="card h-100 border-0">
                        <div class="card-header border-bottom"><h3 class="card-title fw-bold">Distribuição</h3></div>
                        <div class="card-body d-flex align-items-center justify-content-center"><div id="expenses-chart" class="w-100"></div></div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-content border-0 rounded-4 shadow-lg">
          <form action="save_expense.php" method="POST" enctype="multipart/form-data">
              ...
              <div class="modal-body p-4">
                ...
                <div class="row g-3">
                    <div class="col-6"><label class="small fw-bold text-muted">Data</label><input type="date" name="date" id="date" class="form-control-custom" value="<?php echo date('Y-m-d'); ?>" required></div>
                    <div class="col-6"><label class="small fw-bold text-muted">Valor (€)</label><input type="number" step="0.01" name="amount" id="amount" class="form-control-custom fw-bold" placeholder="0.00" required></div>
                    <div class="col-12"><label class="small fw-bold text-muted">Categoria</label><select name="category" id="category" class="form-select-custom" required><option value="Combustível">⛽ Combustível</option><option value="Manutenção">🔧 Manutenção</option><option value="Pessoal">👔 Pessoal</option><option value="Portagens">🛣️ Portagens</option><option value="Outros">📝 Outros</option></select></div>
                    <div class="col-12"><label class="small fw-bold text-muted">Descrição</label><input type="text" name="description" id="description" class="form-control-custom" placeholder="Ex: Gasóleo Carrinha 1" required></div>
                    <div class="col-12"><label class="small fw-bold text-muted">Comprovativo</label><input type="file" name="proof" class="form-control-custom" accept="image/*,application/pdf"></div>
                </div>
              </div>
              <div class="modal-footer border-top-0 pt-0 px-4 pb-4"><button type="submit" class="btn btn-modern w-100 rounded-pill shadow-sm">Guardar</button></div>
          </form>
        </div>
      </div>
    </div>
    ...
    <script>
      ...
      themeToggle.addEventListener('click', () => {
          ...
          updateThemeIcon(newTheme); updateLogo(newTheme); updateChartTheme(newTheme);
      });
      ...
      $(document).ready(function() {
            if ($(window).width() >= 992) {
                $('#tabelaDespesas').DataTable({ "language": { "url": "//cdn.datatables.net/plug-ins/1.13.6/i18n/pt-PT.json", search: "", searchPlaceholder: "Procurar..." }, "pageLength": 10, "lengthChange": false, "searching": false, "ordering": false, "dom": 'tp' });
            }
            ...
            $(document).on('click', '.edit-btn', function() {
                $('#modalTitle').text('Editar Despesa'); $('#expense_id').val($(this).data('id')); $('#date').val($(this).data('date')); $('#category').val($(this).data('cat')); $('#description').val($(this).data('desc')); $('#amount').val($(this).data('amount'));
                new bootstrap.Modal(document.getElementById('modalExpense')).show();
            });
            $('#modalExpense').on('hidden.bs.modal', function(){ $(this).find('form').trigger('reset'); $('#modalTitle').text('Nova Despesa'); $('#expense_id').val(''); });
            ...
            if(success) { const newUrl = window.location.protocol + "//" + window.location.host + window.location.pathname + "?month=<?php echo $mesFiltro; ?>"; window.history.replaceState({path: newUrl}, '', newUrl); }
      });

      function updateChartTheme(theme) { if(expenseChart) { expenseChart.updateOptions({ theme: { mode: theme }, legend: { labels: { colors: theme === 'dark' ? '#cbd5e1' : '#374151' } } }); } }
      ...
      function openPreview(path, ext) {
          ...
          if(ext==='pdf'){ $('#previewFrame').attr('src', path).removeClass('d-none'); } else { $('#previewImage').attr('src', path).removeClass('d-none'); }
          m.show();
      }
    </script>
  </body>
