...
if (file_put_contents($full_server_path, $image_base64)) {
    ...
    try {
        ...
        // C. Enviar Email
        escreverLog("A iniciar envio para destinatário de teste. ID: $trip_id");
        $mail = new PHPMailer(true);

        try {
            // --- NOVAS CONFIGURAÇÕES SMTP ---
            $mail->isSMTP();
            $mail->Host       = 'cloud865.thundercloud.uk'; // Novo Host
            $mail->SMTPAuth   = true;
            $mail->Username   = 'no-reply@syncride.wmservers.pt'; // Novo User
            $mail->Password   = 'KseZ@oMWo%nV'; // Nova Password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
            $mail->Port       = 465;

            // Remetente (Atualizado para o novo domínio)
            $mail->setFrom('no-reply@syncride.wmservers.pt', 'Welcome Agitation  Alertas');
            
            // --- MUDANÇA: APENAS EMAIL DE TESTE ---
           ...
           $mail->addAddress('flexewar@gmail.com');
            ...
            escreverLog("SUCESSO: Email enviado para tiagofsilva04@gmail.com!");
