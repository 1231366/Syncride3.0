...
<head>
    ...
    <style>
        ...
        /* --- NAVBAR & SIDEBAR --- */
        .app-header { background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border-color); height: 70px; }
        [data-bs-theme="dark"] .app-header { background: rgba(30, 41, 59, 0.85); }

        .app-sidebar { background-color: var(--bg-card); border-right: 1px solid var(--border-color); box-shadow: var(--shadow-sm); }
        ...
        .sidebar-menu .nav-link { color: var(--text-muted); border-radius: var(--radius-sm); margin: 4px 12px; padding: 10px 16px; font-weight: 500; transition: all 0.2s; }
        ...
        /* Correção para Input Group (Varinha Mágica) ficar ao lado */
        .input-group .form-control-custom {
            width: auto; /* Deixa o flexbox decidir a largura */
            flex: 1 1 auto;
        }
        ...
        /* --- BOTTOM NAV & BUTTONS --- */
        .bottom-navbar { background-color: var(--bg-card); border-top: 1px solid var(--border-color); z-index: 1050; padding-bottom: env(safe-area-inset-bottom); }
        ...
        .btn-modern { background-color: var(--primary-accent); color: #fff; border: none; border-radius: var(--radius-sm); padding: 0.75rem 1.5rem; font-weight: 500; transition: background 0.2s; }
        ...
        .table thead th { background-color: var(--table-head-bg) !important; color: var(--text-muted); font-weight: 600; text-transform: uppercase; font-size: 0.75rem; border-bottom: 2px solid var(--border-color); }
...
<body class="layout-fixed sidebar-expand-lg">
    <div class="app-wrapper">
        ...
        <div class="sidebar-wrapper mt-3">
          <nav>
            <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="menu" data-accordion="false">
              ...
              <li class="nav-item"><a href="manageUsers.php" class="nav-link active"><i class="nav-icon bi bi-people-fill"></i><p>Equipa</p></a></li>
      ...
      <div class="bottom-navbar position-fixed bottom-0 w-100 d-lg-none d-flex justify-content-around align-items-center shadow-lg">
        ...
        <a href="manageUsers.php" class="nav-item-bottom active text-decoration-none d-flex flex-column align-items-center"><i class="bi bi-people-fill"></i><span>Equipa</span></a>
        ...
        <div class="app-content">
          <div class="container-fluid">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-4 gap-3">
                <div><h3 class="fw-bold mb-0 text-main">Gestão de Equipa</h3><p class="text-muted mb-0 small">Administradores, Condutores e Parceiros.</p></div>
            </div>

            <div class="row g-4 mb-4">
              <div class="col-lg-3 col-6"><div class="stat-card stat-blue"><div><div class="stat-icon-wrapper"><i class="bi bi-person-badge-fill"></i></div><div class="stat-value"><?= $totalAdmins ?></div><div class="stat-label">Administradores</div></div></div></div>
              <div class="col-lg-3 col-6"><div class="stat-card stat-green"><div><div class="stat-icon-wrapper"><i class="bi bi-car-front-fill"></i></div><div class="stat-value"><?= $totalDrivers ?></div><div class="stat-label">Condutores</div></div></div></div>
              <div class="col-lg-3 col-6"><div class="stat-card stat-purple"><div><div class="stat-icon-wrapper"><i class="bi bi-shop"></i></div><div class="stat-value"><?= $totalPartners ?></div><div class="stat-label">Parceiros</div></div></div></div>
              <div class="col-lg-3 col-6"><div class="stat-card stat-orange clickable" data-bs-toggle="modal" data-bs-target="#modalCriarUtilizador"><div><div class="stat-icon-wrapper"><i class="bi bi-person-plus-fill"></i></div><div class="stat-value" style="font-size: 1.5rem;">Novo</div><div class="stat-label">Criar Conta</div></div><div class="text-end text-muted"><i class="bi bi-arrow-right"></i></div></div></div>
            </div>

            <div class="row">
                <div class="col-12">
                    <div class="card border-0">
                        <div class="card-header d-flex justify-content-between align-items-center"><h3 class="card-title fw-bold">Colaboradores</h3></div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover align-middle mb-0">
                                    <thead>
                                        <tr><th class="ps-4">Nome</th><th class="d-none d-lg-table-cell">Email</th><th class="d-none d-lg-table-cell">Telefone</th><th class="d-none d-lg-table-cell">Cargo</th><th class="text-end pe-4">Ações</th></tr>
                                    </thead>
                                    <tbody>
                                            ...
                                            <tr>
                                                <td class="ps-4">
                                                    <div class="d-flex align-items-center">
                                                        <div class="avatar-initial rounded-circle bg-primary-subtle text-primary fw-bold d-flex align-items-center justify-content-center me-3" style="width: 40px; height: 40px;"><?= strtoupper(substr($user['name'], 0, 1)) ?></div>
                                                        <div><div class="fw-bold text-main"><?= htmlspecialchars($user['name']) ?></div><div class="d-lg-none mt-1"><?= $roleBadge ?></div></div>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="text-end pe-4">
                                                    <div class="d-flex justify-content-end gap-2">
                                                        <button class="btn-action" data-bs-toggle="modal" data-bs-target="#editUserModal" data-id="<?= $user['id'] ?>" data-name="<?= htmlspecialchars($user['name']) ?>" data-email="<?= htmlspecialchars($user['email']) ?>" data-phone="<?= htmlspecialchars($user['phone']) ?>" data-role="<?= $user['role'] ?>" title="Editar">
                                                            <i class="bi bi-pencil"></i>
                                                        </button>
                                                        <button class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteUserModal" data-id="<?= $user['id'] ?>" data-name="<?= htmlspecialchars($user['name']) ?>" title="Apagar">
    ...
    <div class="modal fade" id="modalCriarUtilizador" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header border-bottom-0 pb-0"><h5 class="modal-title fw-bold text-main ps-2">Criar Utilizador</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body p-4">
                    <form action="createUser.php" method="POST">
                        <div class="mb-3"><label class="form-label small fw-bold text-muted">Nome</label><input type="text" class="form-control-custom" name="name" required /></div>
                        <div class="mb-3"><label class="form-label small fw-bold text-muted">Email</label><input type="email" class="form-control-custom" name="email" required /></div>
                        <div class="mb-3">
                            <label class="form-label small fw-bold text-muted">Password</label>
                            <div class="input-group">
                                <input type="text" class="form-control-custom" id="inputPassword" name="password" required readonly style="border-top-right-radius: 0; border-bottom-right-radius: 0; border-right: 0;" />
                                <button type="button" class="btn" id="generatePasswordBtn" style="border: 1px solid var(--input-border); border-left: 0; background-color: var(--input-bg); border-top-right-radius: var(--radius-sm); border-bottom-right-radius: var(--radius-sm); color: var(--primary-accent);"><i class="bi bi-magic"></i></button>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6 mb-3"><label class="form-label small fw-bold text-muted">Telefone</label><input type="text" class="form-control-custom" name="phone" required /></div>
                        ...
                        <div class="d-grid mt-4"><button type="submit" class="btn btn-modern shadow-sm rounded-pill">Criar Conta</button></div>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="editUserModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header border-bottom-0 pb-0"><h5 class="modal-title fw-bold text-main ps-2">Editar Utilizador</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body p-4">
                    <form action="editar_utilizador.php" method="POST">
                        <input type="hidden" id="editUserId" name="id">
                        <div class="mb-3"><label class="form-label small fw-bold text-muted">Nome</label><input type="text" class="form-control-custom" id="editUserName" name="name" required></div>
                        <div class="mb-3"><label class="form-label small fw-bold text-muted">Email</label><input type="email" class="form-control-custom" id="editUserEmail" name="email" required></div>
                        <div class="row">
                             <div class="col-6 mb-3"><label class="form-label small fw-bold text-muted">Telefone</label><input type="text" class="form-control-custom" id="editUserPhone" name="phone" required></div>
                             <div class="col-6 mb-3"><label class="form-label small fw-bold text-muted">Cargo</label>
                                <select class="form-select-custom" id="editUserRole" name="role" required>
                                    <option value="1">Admin</option>
                                    <option value="2">Condutor</option>
                                    <option value="3">Parceiro (Agência)</option> 
                                </select>
                        ...
                        <div class="mb-4"><label class="form-label small fw-bold text-muted">Nova Password (Opcional)</label><input type="password" class="form-control-custom" name="password" placeholder="Manter atual"></div>
                        <div class="d-grid mt-4"><button type="submit" class="btn btn-modern shadow-sm rounded-pill">Guardar Alterações</button></div>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-body text-center p-5">
                    ...
                    <p class="text-muted mb-4">Vai apagar <strong id="deleteUserName" class="text-main"></strong> permanentemente. Esta ação não pode ser desfeita.</p>
    ...
    <script>
        ...
        if (success) { const newUrl = window.location.protocol + "//" + window.location.host + window.location.pathname; window.history.replaceState({path: newUrl}, '', newUrl); }
    </script>
  </body>
</html>